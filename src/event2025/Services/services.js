...
export const getCommitteePageList = async () => {
  const serviceParams = {
    ...
    dataTransformers: {
      transformCommitteeMembers: (data) =>
        data.CommunityDetailList?.map((member) => ({
          name: member.Name,
  ...
  try {
    ...
    if (serviceParams.dataTransformers) {
      extractedData.transformedCommitteeMembers =
        serviceParams.dataTransformers.transformCommitteeMembers(
          response.data.res
        );
    }
...
export const getFacultyPageList = async () => {
  const serviceParams = {
    ...
    dataTransformers: {
      transformFacultyMembers: (data) =>
        data.FacultyDetailsList?.map((member) => ({
          name: member.Name,
          role: member.CategoryName,
          title: member.Designation,
          education: member.Education,
  ...
  try {
    ...
    if (serviceParams.dataTransformers) {
      extractedData.transformedFacultyMembers =
        serviceParams.dataTransformers.transformFacultyMembers(
          response.data.res
        );
    }
...
export const getAwardsPageList = async () => {
    ...
    dataTransformers: {
      transformDescriptionToHtml: (data) => {
        ...
        sections.forEach((section) => {
          if (
            section.trim().startsWith("Eligibility Criteria:") ||
            ...
            const listItems = section
              ...
              .filter(
                (item) =>
                  item.trim().length > 0 &&
                  !item.trim().startsWith("Click Here")
              )
              .map((item) => {
                ...
                if (item.includes("Click Here")) {
                  return item.replace(
                    "Click Here",
                    '<a href="#" class="text-blue-600 hover:underline ml-1">Click Here</a>'
                  );
                }
                return item;
              });
  ...
  try {
    ...
    if (serviceParams.dataTransformers) {
      extractedData.htmlDescription =
        serviceParams.dataTransformers.transformDescriptionToHtml(
          response.data.res
        );
    }
...
export const createPageService = (config) => {
  return async (customParams = {}) => {
    ...
    try {
      ...
      if (serviceParams.dataTransformers) {
        Object.entries(serviceParams.dataTransformers).forEach(
          ([key, transformer]) => {
